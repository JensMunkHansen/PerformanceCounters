...
REM dependencies.bat - Build dependencies for project on Windows
REM Builds Catch2 test framework
...
REM ================================================
if /i not "%TOOLSET%"=="msvc" if /i not "%TOOLSET%"=="clangcl" (
    echo ERROR: Unknown toolset '%TOOLSET%'. Use 'msvc' or 'clangcl'.
    exit /b 1
)
...
REM Set project root environment variable (NativeDeps expects Dependencies_ROOT)
set Dependencies_ROOT=%SCRIPT_DIR%

echo ==============================================
echo Dependencies Builder
echo ==============================================
echo.
...
echo   --config CONFIG   Build only specified config (Debug or Release)
echo                     Default: Debug + Release
